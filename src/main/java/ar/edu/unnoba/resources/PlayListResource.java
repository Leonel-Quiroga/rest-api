...
public class PlayListResource {
	...
	public Response<PlayListDTO> getInformationOfPlaylist(
		...
		try {
			response.setElemento(MapperUtils.map(playlistDAO.getInformationOfPlaylist(idPlayList), PlayListDTO.class));
			response.setSuccess(true);
		} catch (Exception ex) {
			logger.warning("getInformationOfPlaylist: " + ex.getMessage());
			response.setMsg("No se pudo obtener el listado de playlist");
		}
		return response;
	}
